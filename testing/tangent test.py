...
def calc_tangent(splinerep_tck:tuple, u, x_vals, y_vals):
    ...
    for slope,y_int,x in zip(tan_slopes, y_ints, tan_xs):
        tan_ys.append(np.dstack((x,find_tan_y(x, slope, y_int))).tolist())
...
def main():
    ...
    ax.plot(spline_x, spline_y, '-b', label = 'spline')
    
    colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    tans = LineCollection(tangents_y, colors=colors, linestyle='solid')
