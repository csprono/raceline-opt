...
def adjust_bodyfit(occ_grid : np.array) -> np.array:
    '''
        Inputs: 
        ...
        Output:
        - Occupancy grid that depicts drivable area and accounts for width of
          the car
    '''
    shrinkage_amount = 15
...
def separate_l_r(occ_grid : np.array):
    '''
        Inputs:
    ...
    for i in range(len(contours)):
        ...
        if hierarchy[0][i][3] == -1:
            ...
            cv.drawContours(new, contours, i, (0, 0, 255), 2)

            # Outer contour, store its points
            right.append(contours[i])
        else:
            # Inner contour, draw in blue
            cv.drawContours(new, contours, i, (255, 0, 0), 2)
